...
namespace SilverStripe\Internationalisation\Messages\Symfony;
...
class SymfonyMessageProvider implements MessageProvider
{
    ...
    protected function load($locale)
    {
        ...
        $lang = Internationalisation::getData()->langFromLocale($locale);
        if ($lang !== $locale) {
            $this
                ->getTranslator()
    ...
    {
        if (!$this->sourceDirs) {
            $this->setSourceDirs(Internationalisation::getSources()->getLangDirs());
        }
