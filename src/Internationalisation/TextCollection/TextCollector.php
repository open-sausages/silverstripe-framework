...
namespace SilverStripe\Internationalisation\TextCollection;
...
use SilverStripe\Internationalisation\Messages\Reader;
use SilverStripe\Internationalisation\Messages\Writer;
...
class TextCollector
{
    ...
    {
        $defaultLocale = Internationalisation::config()->uninherited('default_locale');
        $this->defaultLocale = $locale ?: Internationalisation::getData()->langFromLocale($defaultLocale);
        $this->basePath = Director::baseFolder();
        $this->baseSavePath = Director::baseFolder();
        $this->setWarnOnEmptyDefault(Internationalisation::config()->uninherited('missing_default_warning'));
    }
            ...
            switch ($token) {
                ...
                case ')':
                    ...
                    if (!empty($currentEntity[0])) {
                        ...
                        if ($default) {
                            $plurals = Internationalisation::parse_plurals($default);
    ...
    public function collectFromEntityProviders($filePath, Module $module = null)
    {
        ...
        foreach ($classes as $class) {
            ...
            if (!class_exists($class) || !is_a($class, EntityProvider::class, true)) {
                continue;
            }
