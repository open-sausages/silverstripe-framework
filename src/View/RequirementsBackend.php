...
class RequirementsBackend
{
    ...
    public function add_i18n_javascript($langDir, $return = false)
    {
        ...
        $files = array();
        $defaultLocale = Internationalisation::config()->get('default_locale');
        $currentLocale = Internationalisation::get_locale();
        $candidates = [
            'en.js',
            'en_US.js',
            Internationalisation::getData()->langFromLocale($defaultLocale) . '.js',
            $defaultLocale . '.js',
            Internationalisation::getData()->langFromLocale($currentLocale) . '.js',
            $currentLocale . '.js',
        ];
    ...
    {
        $path = ThemeResourceLoader::inst()->findThemedCSS($name, Viewer::get_themes());
        if ($path) {
            $this->css($path, $media);
        } else {
            throw new InvalidArgumentException(
    ...
    {
        $path = ThemeResourceLoader::inst()->findThemedJavascript($name, Viewer::get_themes());
        if ($path) {
            $opts = [];
            if ($type) {
                $opts['type'] = $type;
