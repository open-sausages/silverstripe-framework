...
/**
  ...
  */
class TemplateParser extends Parser implements TemplateParserInterface
{
   ...
   !( "if " | "unless " )
    ...
    {
        throw new TemplateParserException('You cant have cache blocks nested within with, loop or control blocks ' .
            'that are within cache blocks', $this);
    }

    function CacheRestrictedTemplate_UncachedBlock(&$res, $sub)
    {
        throw new TemplateParserException('You cant have uncache blocks nested within with, loop or control blocks ' .
            'that are within cache blocks', $this);
    }
    ...
    function CacheBlock_CacheBlockTemplate(&$res, $sub)
    {
        ...
        $res['php'] .= '$val = \'\';' . PHP_EOL;
        if ($globalKey = Viewer::config()->get('global_key')) {
            ...
            if (!$result) {
                throw new TemplateParserException('Unexpected problem parsing template', $parser);
            }
    ...
    function Include__finalise(&$res)
    {
        ...
        $res['php'] = '$val .= \\SilverStripe\\View\\Templates\\SSViewer::execute_template([["type" => "Includes", '.$template.'], '.$template.'], $scope->getItem(), array(' .
    ...
    function ClosedBlock__finalise(&$res)
    {
        ...
        if (method_exists($this, $method)) {
            ...
            throw new TemplateParserException('Unknown closed block "'.$blockname.'" encountered. Perhaps you are ' .
    ...
    function ClosedBlock_Handle_Loop(&$res)
    {
        if ($res['ArgumentCount'] > 1) {
            throw new TemplateParserException('Either no or too many arguments in control block. Must be one ' .
                'argument only.', $this);
        }
        ...
        if ($res['ArgumentCount'] == 0) {
            ...
            if ($arg['ArgumentMode'] == 'string') {
                throw new TemplateParserException('Control block cant take string as argument.', $this);
            }
            $on = str_replace(
                '$$FINAL',
                'obj',
    ...
    {
        if ($res['ArgumentCount'] != 1) {
            throw new TemplateParserException('Either no or too many arguments in with block. Must be one ' .
                'argument only.', $this);
        }
    ...
    function OpenBlock__finalise(&$res)
    {
        ...
        if (method_exists($this, $method)) {
            ...
            throw new TemplateParserException('Unknown open block "'.$blockname.'" encountered. Perhaps you missed ' .
    ...
    function OpenBlock_Handle_Debug(&$res)
    {
        if ($res['ArgumentCount'] == 0) {
            ...
            throw new TemplateParserException('Debug takes 0 or 1 argument only.', $this);
        }
    }
    ...
    {
        if ($res['ArgumentCount'] != 0) {
            throw new TemplateParserException('Base_tag takes no arguments', $this);
        }
        return '$val .= \\SilverStripe\\View\\Templates\\SSViewer::get_base_tag($val);';
    }
    ...
    {
        if ($res['ArgumentCount'] != 0) {
            throw new TemplateParserException('Current_page takes no arguments', $this);
        }
    ...
    function MismatchedEndBlock__finalise(&$res)
    {
        ...
        throw new TemplateParserException('Unexpected close tag end_' . $blockname .
    ...
    {
        $tag = $res['Tag']['text'];
        throw new TemplateParserException("Malformed opening block tag $tag. Perhaps you have tried to use operators?", $this);
    }
    ...
    {
        $tag = $res['Tag']['text'];
        throw new TemplateParserException("Malformed closing block tag $tag. Perhaps you have tried to pass an " .
...
EOC;
    ...
    public function compileString($string, $templateName = "", $includeDebuggingComments = false, $topTemplate = true)
    {
        if (!trim($string)) {
            ...
            if (!$result) {
                throw new TemplateParserException('Unexpected problem parsing template', $this);
            }
