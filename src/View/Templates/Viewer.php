 ...
 */
class Viewer implements Flushable
{
    use Configurable;
    use Injectable;
     ...
     */
    public function __construct($templates, TemplateParserInterface $parser = null)
    {
        if ($parser) {
    ...
    {
        $viewer = ViewerFromString::create($content);
    ...
    {
        $currentThemes = Viewer::get_themes();
    ...
    public static function get_themes()
    {
        ...
        if (!Viewer::config()->uninherited('theme_enabled')) {
            return $default;
        }
        ...
        if (!isset($themes)) {
            $themes = Viewer::config()->uninherited('themes');
        }
        if ($themes) {
            return $themes;
        }
        ...
        if ($theme = Viewer::config()->uninherited('theme')) {
    ...
    {
        if (Viewer::$topLevel) {
            return Viewer::$topLevel[sizeof(Viewer::$topLevel)-1];
        }
        return null;
    }
     ...
     * @param TemplateParserInterface $parser
     */
    public function setParser(TemplateParserInterface $parser)
    {
        $this->parser = $parser;
    }
     ...
     *
     * @return TemplateParserInterface
     */
    public function getParser()
    {
        if (!$this->parser) {
            $this->setParser(Injector::inst()->get('SilverStripe\\View\\Templates\\TemplateParser'));
        }
        return $this->parser;
    }
    ...
    protected function includeGeneratedTemplate($cacheFile, $item, $overlay, $underlay, $inheritedScope = null)
    {
        ...
        $scope = new ViewerDataPresenter($item, $overlay, $underlay, $inheritedScope);
        $val = '';
...
PHP;
    ...
    public function parseTemplateContent($content, $template = "")
    {
        return $this->getParser()->compileString(
            $content,
            $template,
            Director::isDev() && Viewer::config()->uninherited('source_file_comments')
        );
    }
