...
use SilverStripe\View\Templates\ViewerScope;
use SilverStripe\View\Templates\TemplateGlobalProvider;
use SilverStripe\View\Templates\TemplateIteratorProvider;
 ...
 */
class ViewerDataPresenter extends ViewerScope
{
    /**
     ...
     * @var array $overlay
     * @var array $underlay
     * @var ViewerScope $inheritedScope
     */
    public function __construct(
        $item,
        array $overlay = null,
        array $underlay = null,
        ViewerScope $inheritedScope = null
    ) {
    ...
    public function pushScope()
    {
        ...
        if ($upIndex !== null) {
            ...
            $itemStack[$upIndex][ViewerScope::ITEM_OVERLAY] = $this->overlay;
    ...
    public function popScope()
    {
        ...
        if ($upIndex !== null) {
            $itemStack = $this->getItemStack();
            $this->overlay = $itemStack[$this->getUpIndex()][ViewerScope::ITEM_OVERLAY];
        }
    ...
    public function obj($name, $arguments = [], $cache = false, $cacheName = null)
    {
        ...
        if ($overlayIndex !== false) {
            $itemStack = $this->getItemStack();
            if (!$this->overlay && isset($itemStack[$overlayIndex][ViewerScope::ITEM_OVERLAY])) {
                $this->overlay = $itemStack[$overlayIndex][ViewerScope::ITEM_OVERLAY];
            }
        }
