...
For instance, in order to write an asset to a secure location you can use the following additional
config option:


    :::php
    $store = singleton('AssetStore');
    $store->setFromString('My secure content', 'Documents/Mydocument.txt', null, null, array(
...
Access for files is granted on a per-session bases, rather than on a per-member basis, via a
...
## How file access is secured

Public urls to files do not change, regardless of whether the file is secure or public. Similarly,
operations which modify files do not normally need to be told whether the file is secure or public
...
the page is published, but will be secure in the mean time.
...
If you are deploying your site to a server configuraion that makes use of static caching, it's essential
...
cache protected content being served to authenticated users. This can be achived by configuring your cache
correctly to skip `Pragma: no-cache` headers and the `bypassStaticCache` cookie.

## Configuring secure assets

### Configuring: Secure folder location

In the default silverstripe configuration, secure assets are placed within the web root into the
...
In order to better ensure these files are secured, it's recommended to move this outside of the web
root altogether.
...
### Configuring: Secure file headers
...
When a secure file is served it will also be transmitted with all headers defined by the
...
rather than deleted. This use more disk storage, but will allow the full recovery of archived
records and files.
...
(declared by `Assets_HTAccess.ss`) has the following section, wich may be customised in your project:
...
while ensuring non-existant files are processed via framework.
...
dynamic requests are processed via your framework:
